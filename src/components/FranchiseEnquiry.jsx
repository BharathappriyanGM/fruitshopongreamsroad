  ...
  return (
    <div className="franchise-bg-page">
      ...
      <motion.nav
        ...
        style={{
          background: "#FFFFF", backdropFilter: "blur(28px)", WebkitBackdropFilter: "blur(28px)",
        ...
        <a href="/" style={{ display: "flex", alignItems: "center", gap: 2, textDecoration: "none" }}>
          <img src="/logo.png" alt="" style={{ width: 52, height: 52, borderRadius: "50%", objectFit: "cover", marginRight: 3 }} />
          <div>
            <img src="/logo-name.png" alt="Fruit Shop on Greams Road" style={{ height: 55, width: "auto", marginTop: 7 }} />
          </div>
        </a>
        ...
        <ul style={{ display: "flex", alignItems: "center", gap: 32, listStyle: "none", margin: 0, padding: 0 }} className="nav-links">
          <li>
            <a
              ...
              style={{ color: "#ffffff", fontWeight: 600, fontSize: 14, textDecoration: "none", cursor: "pointer", fontFamily: "'Poppins', sans-serif" }}
            >
              Menu
            </a>
          </li>
          ...
          <li>
            <a
              onClick={() => { navigate("/"); setTimeout(() => document.getElementById("locations")?.scrollIntoView({ behavior: "smooth" }), 100); }}
              style={{ color: "#ffffff", fontWeight: 600, fontSize: 14, textDecoration: "none", cursor: "pointer", fontFamily: "'Poppins', sans-serif" }}
            >
              Locations
            </a>
      ...
      <div className="franchise-page-inner" style={{ maxWidth: 1100, margin: "0 auto", padding: "64px 48px", display: "grid", gridTemplateColumns: "1.1fr 0.9fr", gap: 80, alignItems: "start" }}>
        ...
        <motion.div initial={{ opacity: 0, y: 32 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
          <span style={{ display: "inline-block", background: "rgba(192,133,82,0.12)", color: "#C08552", fontSize: 14, fontWeight: 700, letterSpacing: 2, textTransform: "uppercase", padding: "6px 16px", borderRadius: 100, marginBottom: 16, fontFamily: "'Poppins', sans-serif" }}>
            Franchise
          </span>
          <h2 style={{ fontSize: "clamp(32px,4vw,48px)", fontWeight: 900, color: "#4B2E2B", lineHeight: 1.15, marginBottom: 8, fontFamily: "'Merienda', serif" }}>
            Own a Slice of<br /><em style={{ color: "#C08552" }}>Tamil Nadu's Freshest</em>
          </h2>
          <p style={{ fontSize: 14, color: "#fff", fontWeight: 600, lineHeight: 1.7, marginBottom: 32, fontFamily: "'Poppins', sans-serif" }}>
          ...
          <div style={{ background: "rgba(255,255,255,0.5)", backdropFilter: "blur(22px)", WebkitBackdropFilter: "blur(22px)", border: "1px solid rgba(255,255,255,0.85)", borderRadius: 24, padding: 32, boxShadow: "0 8px 40px rgba(75,46,43,0.1)" }}>
            {submitted ? (
              <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }}
                ...
                <div style={{ fontSize: 22, color: "#4B2E2B", fontWeight: 700, marginBottom: 8, fontFamily: "'Merienda', serif" }}>Your dream just took a step closer!</div>
                <p style={{ color: "#2C1810", fontSize: 15, lineHeight: 1.6, fontFamily: "'Poppins', sans-serif", fontWeight: 500 }}>We've received your enquiry. Our team will squeeze some time to reach you shortly.</p>
              </motion.div>
            ) : (
              <form onSubmit={handleSubmit}>
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, marginBottom: 16 }}>
                  {[["Name *", "name", "text", "Your full name"], ["Email *", "email", "email", "your@email.com"]].map(([label, key, type, ph]) => (
                    <div key={key}>
                      <label style={{ display: "block", fontSize: 13, fontWeight: 800, color: "#4B2E2B", marginBottom: 7, fontFamily: "'Poppins', sans-serif" }}>{label}</label>
                      <input required type={type} placeholder={ph} value={form[key]} onChange={set(key)}
                        style={{ width: "100%", padding: "13px 16px", border: "1.5px solid rgba(192,133,82,0.3)", borderRadius: 12, fontSize: 14, outline: "none", background: "rgba(255,255,255,0.65)", backdropFilter: "blur(8px)", WebkitBackdropFilter: "blur(8px)", fontFamily: "'Poppins', sans-serif", fontWeight: 500, boxSizing: "border-box" }} />
                    </div>
                  ))}
                </div>
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, marginBottom: 16 }}>
                  <div>
                    <label style={{ display: "block", fontSize: 13, fontWeight: 800, color: "#4B2E2B", marginBottom: 7, fontFamily: "'Poppins', sans-serif" }}>Contact *</label>
                    <input required placeholder="+91 XXXXXXXXXX" value={form.contact} onChange={set("contact")}
                      style={{ width: "100%", padding: "13px 16px", border: "1.5px solid rgba(192,133,82,0.3)", borderRadius: 12, fontSize: 14, outline: "none", background: "rgba(255,255,255,0.65)", backdropFilter: "blur(8px)", WebkitBackdropFilter: "blur(8px)", fontFamily: "'Poppins', sans-serif", fontWeight: 500, boxSizing: "border-box" }} />
                  </div>
                  <div>
                    <label style={{ display: "block", fontSize: 13, fontWeight: 800, color: "#4B2E2B", marginBottom: 7, fontFamily: "'Poppins', sans-serif" }}>State *</label>
                    <select required value={form.state} onChange={set("state")}
                      style={{ width: "100%", padding: "13px 16px", border: "1.5px solid rgba(192,133,82,0.3)", borderRadius: 12, fontSize: 14, outline: "none", background: "rgba(255,255,255,0.65)", backdropFilter: "blur(8px)", WebkitBackdropFilter: "blur(8px)", appearance: "none", fontFamily: "'Poppins', sans-serif", fontWeight: 500, boxSizing: "border-box", color: form.state ? "#2C1810" : "#9ca3af", cursor: "pointer" }}>
                ...
                <div style={{ marginBottom: 16 }}>
                  <label style={{ display: "block", fontSize: 13, fontWeight: 800, color: "#4B2E2B", marginBottom: 7, fontFamily: "'Poppins', sans-serif" }}>Interested City / Location *</label>
                  <input required placeholder="City or identified location" value={form.city} onChange={set("city")}
                    style={{ width: "100%", padding: "13px 16px", border: "1.5px solid rgba(192,133,82,0.3)", borderRadius: 12, fontSize: 14, outline: "none", background: "rgba(255,255,255,0.65)", backdropFilter: "blur(8px)", WebkitBackdropFilter: "blur(8px)", fontFamily: "'Poppins', sans-serif", fontWeight: 500, boxSizing: "border-box" }} />
                </div>
                <div style={{ marginBottom: 20 }}>
                  <label style={{ display: "block", fontSize: 13, fontWeight: 800, color: "#4B2E2B", marginBottom: 7, fontFamily: "'Poppins', sans-serif" }}>Message</label>
                  <textarea placeholder="Tell us about yourself and why you'd be a great franchise partner..." value={form.message} onChange={set("message")}
                    style={{ width: "100%", padding: "13px 16px", border: "1.5px solid rgba(192,133,82,0.3)", borderRadius: 12, fontSize: 14, outline: "none", background: "rgba(255,255,255,0.65)", backdropFilter: "blur(8px)", WebkitBackdropFilter: "blur(8px)", minHeight: 110, resize: "vertical", fontFamily: "'Poppins', sans-serif", fontWeight: 500, boxSizing: "border-box" }} />
                </div>
                {error && (
                  <div style={{ background: "#fef2f2", border: "1px solid #fecaca", borderRadius: 10, padding: "10px 16px", marginBottom: 16, color: "#dc2626", fontSize: 13, fontFamily: "'Poppins', sans-serif", fontWeight: 600 }}>
                    {error}
                  </div>
                )}
                <motion.button type="submit" disabled={loading} whileHover={{ y: -2, boxShadow: "0 12px 32px rgba(75,46,43,0.25)" }} whileTap={{ scale: 0.97 }}
                  style={{ width: "100%", background: loading ? "#8C5A3C" : "#4B2E2B", color: "#fff", border: "none", padding: "15px 40px", borderRadius: 100, fontSize: 15, fontWeight: 700, cursor: loading ? "not-allowed" : "pointer", fontFamily: "'Poppins', sans-serif" }}>
        ...
        <motion.div initial={{ opacity: 0, x: 32 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.6, delay: 0.2 }}
          ...
          <div style={{ fontSize: 26, fontWeight: 900, color: "white", lineHeight: 1.2, fontFamily: "'Merienda', serif" }}>
            Fruit Shop<br />on Greams Road
          </div>
          <div style={{ fontSize: 12, color: "#fff", fontWeight: 600, letterSpacing: "1.5px", textTransform: "uppercase", fontFamily: "'Poppins', sans-serif" }}>
            Est. 1992 · Tamil Nadu
          </div>
          <p style={{ fontSize: 14, color: "rgba(255,255,255,0.65)", lineHeight: 1.8, fontWeight: 400, fontFamily: "'Poppins', sans-serif" }}>
            Pure, natural, full-bodied, nothing artificial. Other outlets use reconstituted juice and add preservatives. We never will.
          </p>
          <div style={{ display: "flex", flexWrap: "wrap", justifyContent: "center", gap: 8 }}>
            {["100% Natural", "No Preservatives", "No Gas Added", "Fresh Daily", "30+ Yrs Trust"].map(b => (
              <span key={b} style={{ background: "rgba(255,255,255,0.1)", color: "rgba(255,255,255,0.85)", padding: "6px 14px", borderRadius: 100, fontSize: 11, fontWeight: 600, border: "1px solid rgba(255,255,255,0.15)", fontFamily: "'Poppins', sans-serif" }}>{b}</span>
            ))}
          </div>
        </motion.div>
      </div>
